...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None, use_ori=False, disable_aug=[]):
        super().__init__()
        ...
        # Remove augmentor from aug list
        self.data_augmentor = DataAugmentor(
            self.root_path, self.dataset_cfg.DATA_AUGMENTOR, self.class_names, logger=self.logger, extra_disable_aug=disable_aug
    ...
    @staticmethod
    def collate_batch(batch_data, _unused=False):
        if type(batch_data[0]) == tuple:
            size = len(batch_data[0])
            process_list = []

            for i in range(size):
                process_list.append([data[i] for data in batch_data])
            flag = True
        else:
            process_list = [batch_data]
            flag = False
        ret_list = []
        for batch_list in process_list:
            data_dict = defaultdict(list)
            for cur_sample in batch_list:
                #assert 'points'  in cur_sample.keys()
                #print(cur_sample['points'].shape, cur_sample['frame_id'])
            ...
            #print('coll', ret['points'][:,0].max(), ret['frame_id'], data_dict['points'][0].shape[0], data_dict['points'][1].shape[0], data_dict['points'][2].shape[0], data_dict['points'][3].shape[0])
            if flag:
                ret_list.append(ret)
            else:
                return ret
