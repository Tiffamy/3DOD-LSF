...
class KittiDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None, use_ori=False, beam_tag='default'):
        """
        Args:
            root_path:
            dataset_cfg:
    ...
    def get_lidar(self, idx):
        if self.use_ori:
            lidar_file = self.root_split_path / 'modes' / self.beam_tag / ('%s.bin' % idx)
        else:
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            mask = downsample_utils.generate_mask(phi, beam, label, idxs, beam_ratio=2, bin_ratio=1)
            save_downsample = points[mask]
            save_path = self.root_split_path / 'modes' / '32' / ('%s.bin' % sample_idx)
            save_downsample.tofile(save_path)

            mask = downsample_utils.generate_mask(phi, beam, label, idxs, beam_ratio=2, bin_ratio=2)
            save_downsample = points[mask]
            save_path = self.root_split_path / 'modes' / '32^' / ('%s.bin' % sample_idx)
            save_downsample.tofile(save_path)

            mask = downsample_utils.generate_mask(phi, beam, label, idxs, beam_ratio=4, bin_ratio=1)
            save_downsample = points[mask]
            save_path = self.root_split_path / 'modes' / '16' / ('%s.bin' % sample_idx)
            save_downsample.tofile(save_path)

            mask = downsample_utils.generate_mask(phi, beam, label, idxs, beam_ratio=4, bin_ratio=2)
            save_downsample = points[mask]
            save_path = self.root_split_path / 'modes' / '16^' / ('%s.bin' % sample_idx)
            save_downsample.tofile(save_path)
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    os.makedirs('{}/training/modes/32'.format(str(save_path)), exist_ok=True)
    os.makedirs('{}/training/modes/32^'.format(str(save_path)), exist_ok=True)
    os.makedirs('{}/training/modes/16'.format(str(save_path)), exist_ok=True)
    os.makedirs('{}/training/modes/16^'.format(str(save_path)), exist_ok=True)
    ...
    os.system('mv {}/training/velodyne {}/training/modes/64'.format(str(save_path), str(save_path)))
