...
class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None, use_ori=False, disable_aug=[], beam_tag='default'):
        super().__init__(
            dataset_cfg=dataset_cfg, class_names=class_names, training=training, root_path=root_path, logger=logger, use_ori=use_ori, disable_aug=disable_aug
        )
    ...
    def include_waymo_data(self, mode):
        ...
        for k in range(len(self.sample_sequence_list)):
            ...
            info_path = self.root_path / 'processed_data' / sequence_name / ('%s.pkl' % sequence_name)
    ...
    def get_lidar(self, sequence_name, sample_idx):
        if self.use_ori:
            lidar_file = self.root_path / 'modes' / self.beam_tag / sequence_name / ('%04d.npy' % sample_idx)
        else:
